...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardStyle = useMemo(
    ...
    [iconUrl, iconSize, grainUrl, showBehindGradient, behindGradient, innerGradient]
  );

  const handleContactClick = useCallback(() => {
    if (!contactUrl) return;
    try {
      if (contactUrl.startsWith('mailto:') || contactUrl.startsWith('tel:')) {
        window.location.href = contactUrl;
      } else if (contactUrl.startsWith('http') || contactUrl.startsWith('/')) {
        router.push(contactUrl);
      }
    } catch (err) {
      console.error('Failed to navigate contactUrl:', err);
    }
  }, [contactUrl, router]);
